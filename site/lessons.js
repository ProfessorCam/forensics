...
var SITE = {
  ...
  welcome: {
    lead: {
      ...
      e: 'One libpcap file: 1602 frames, 25 seconds, taken from a mirror port on the lab LAN. Three flag fragments, carried by DNS, by HTTP, and by a G.711 RTP stream set up by SIP.'
    },
    story: [
...
var LESSONS = [
  /* ================================================================== A: DNS */
  {
    ...
    facts: [
      ...
      ['Skill', { s: 'Filtering: making 1602 packets into 10', m: 'Display filters; reading a DNS answer', e: 'DNS record types and RDATA' }]
    ],
    sections: [
      { h: 'What happened',
        p: [
    ...
    captures: [{ file: 'evidence.pcap', title: 'The DNS packets', filter: 'dns',
      hint: { s: 'The filter box already says <code>dns</code>, so only the ten DNS packets are shown. Press Clear to see all 1602 again.', m: 'Pre-filtered on <code>dns</code>. Try <code>dns.flags.response == 1</code> for answers only, or <code>dns.qry.name contains lab</code>.', e: 'Filter <code>dns</code>; 10 of 1602.' } }]
  },
  ...
  /* ================================================================== C: SIP + RTP */
  {
    ...
    oneLiner: {
      ...
      m: 'Lab PC 2 called the front desk phone. SIP set the call up and RTP carried the voice, 771 packets each way. Play the call and listen.',
      e: 'SIP dialog on UDP/5060 (INVITE, 100, 180, 200, ACK; BYE, 200). SDP offers PCMU on 192.168.110.60:16384, answer 192.168.110.72:20002. Two RTP streams, PT 0, 20 ms per packet, 15.4 s, no loss.'
    },
    facts: [
